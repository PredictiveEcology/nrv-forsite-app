...
lookup_element_name_by_value <- function(x, value) {
  if (length(value) > 1L) {
    paste0("`value` should be length 1, not length ", length(value), ".") |> stop()
  }
...
lookup_element_type_by_value <- function(x, value) {
  out <- sapply(x, function(y) value %in% y)
...
get_period_choices <- function(x) {
  out <- seq(from = x$year_start, to = x$year_end, by = x$year_interval)
  ...
  shiny::div(
    ...
    shiny::div(
      ...
      shiny::div(
        ...
        shiny::div(class = "accordion-body", content)
      )
    )
  )
}
...
#' in a coloured box: <https://getbootstrap.com/docs/5.0/components/alerts/>.
#'
...
build_alert <- function(type = "warning", content) {
  shiny::div(class = glue::glue("alert alert-{type}"), role = "alert", shiny::HTML(content))
}
