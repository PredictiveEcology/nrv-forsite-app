...
base_map <- function() {
  ...
  leaflet::leaflet() |>
    ...
    leaflet::addTiles(urlTemplate = "", attribution = map_attr) |>
...
#'
#' @noRd
get_palette <- function(n = 50, type = c("viridis", "rdylbu", "spectral", "bam")) {
  switch(
    match.arg(type),
...
add_element <- function(
  ...
  # Build the palette
  palette <- get_palette(n = palette_length, type = palette_type)
  ...
  m <- map |>
    leafem::addGeotiff(
      ...
      autozoom = FALSE,
      layerId = "raster",
      options = leaflet::tileOptions(maxNativeZoom = 10, zIndex = 400),
      colorOptions = leafem::colorOptions(palette = palette, na.color = "transparent")
    )
...
add_element2x <- function(
  ...
  if (compare_by == "scenario") {
    ...
    url <- lapply(list(left_map, right_map), function(x) {
      make_api_path(
    ...
    id1 <- lookup_element_name_by_value(x = SCENARIOS, value = left_map)
    id2 <- lookup_element_name_by_value(x = SCENARIOS, value = right_map)
  } else {
    url <- lapply(list(left_map, right_map), function(x) {
      make_api_path(
        root = paste0(get_golem_config("app_baseurl"), "api"),
  ...
  m <- map |>
    leafem::addGeotiff(
      ...
      options = leaflet::tileOptions(pane = "left", maxNativeZoom = 10, zIndex = 400),
    ...
    leafem::addGeotiff(
      ...
      options = leaflet::tileOptions(pane = "right", maxNativeZoom = 10, zIndex = 400),
      colorOptions = leafem::colorOptions(
